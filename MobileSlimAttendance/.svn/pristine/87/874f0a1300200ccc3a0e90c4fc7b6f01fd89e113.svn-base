...
<script type="text/javascript" src="scripts/futureAssignment.js?updated=<%=new Date()%>"></script>
<link href="styles/demo_table.css" rel="stylesheet" type="text/css">	
</head>
<%@include file="common/assignmentPageUpper.jsp"%>
<form action="futureAssignment" method="post" onsubmit="return validate('<%=request.getSession().getAttribute("isDepartment")%>');">
				...
				<c:forEach var="assignment" items="${assignmentList}">
					<tr <%if (count % 2 == 0) {%> class='altrow' <%}%>>
						...
						<td>
							...
							</div> <input type="hidden" name="shiftId"
