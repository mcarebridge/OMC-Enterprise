...
<!-- Load Tables -->
		function drawVisualization() {
		  ...
		  data.addColumn('string', 'Week No');//0
		  data.addColumn('string', 'Week Start Date'); //1
		  data.addColumn('string', 'Week End Date');//2
		  data.addColumn('string', 'Download Report');//3 	
		 
		  var i =0;
		  
		  <c:forEach var="weeklyReport" items="${requestScope.weeklyReportList}">
		  	data.addRows(1);
		  
	  		data.setCell(i, 0,"${f:h(weeklyReport.monthName)}");
	  		data.setCell(i, 1,"${f:h(weeklyReport.weekNo)}");
	  		data.setCell(i, 2,"${f:h(weeklyReport.startDate)}");
	  		data.setCell(i, 3,"${f:h(weeklyReport.endDate)}");
	  				
	  		<c:if test="${weeklyReport.noOfRecords!=0}">
	  		data.setCell(i,4,"<a href='javascript:fnGetDataTablesData(${f:h(weeklyReport.startTime)});' style='text-decoration: none;'>Download (${f:h(weeklyReport.noOfRecords)} Records)</a> ");
	  		</c:if>
	  		<c:if test="${weeklyReport.noOfRecords==0}">
	  		data.setCell(i,4,"");
	  		</c:if>
		 ...
		 var dashboard=new google.visualization.Dashboard(document.getElementById('weeklyReport'));
...
<form action="weeklyTimeRecord" method="post">

	<input type="hidden" name="actionParam" value="weeklyTimeRecord">
	<input type="hidden" name="wwww" value=""> <input
		type="hidden" name="timeSheetIds">


	<div id="weeklyReport" style="width: 100%">
